...
用户在讨论某个科研项目 skill 重构时明确的 skill 设计偏好：
...
4. **scripts/ 仅用于 skill 自动化**：`scripts/` 放 skill 本身的自动化脚本（如 `init_plan.sh`），**不放训练代码包装器**。训练代码是你的代码仓库的职责，skill 只在 references 里记录 SSH wrapper pattern
5. **已知坑点提取到 references/known-issues.md**：持续增长的内容不应膨胀 SKILL.md

**Why:** 用户追问了 `scripts/launch_train.sh` 的必要性——"我的代码仓库不是本身就有训练用的代码？" 这说明用户重视 skill 与代码仓库的职责边界，不希望 skill 越俎代庖。

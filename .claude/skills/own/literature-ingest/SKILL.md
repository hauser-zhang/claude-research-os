...
如果用户没有给具体文件路径，默认在 `$HOME/Downloads` 中谨慎搜索候选文件。搜索时优先用 paper slug、短标题、第一作者、关键词（如 `smith-2026-example-method`、`example method`、`patch_notes`）匹配，并按修改时间倒序查看。不要使用真实用户主目录写进 skill 或模板；对外描述时用 `$HOME/Downloads`。如果命中多个候选，先报告候选列表并让用户确认；如果只命中一个，也要在合并前说明将使用哪个文件，避免把别的论文 patch 错合进来。

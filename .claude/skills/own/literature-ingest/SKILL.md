...
默认使用 **full-file rewrite**：适合 `stub` / `draft` 页面、刚从 survey 生成的粗读笔记、用户希望外部 GPT 带着论文从头补全的情况。

使用 **patch-notes**：适合 `mature` 页面、已有大量人工判断、担心整篇替换覆盖细节，或用户明确要求只给修改建议。

具体模板见 `references/external-gpt-full-file-rewrite.md`。插入前按 paper 名称、来源、是否有代码仓库、关键术语和必填 section 替换占位符。
...
然后让 GPT 按页面里的 External GPT prompt 输出。
默认要求它输出完整 Markdown；如果你不想整篇替换，就让它改用 patch-notes 模式。
```

...
做完一个研究后最痛的事情之一是"怎么把它放到飞书 / Notion / Confluence 上，让未来的我还能读懂？"Research OS 自带一整套外部平台镜像工作流 [`.claude/rules/feishu-mirror-workflow.md`](.claude/rules/feishu-mirror-workflow.md)：文档类型规范、Writing Material 五层结构、引用三步验证铁律；还有独立的图表风格规范 [`figure-style-guidelines.md`](.claude/rules/figure-style-guidelines.md)。告诉 Claude"把这个 thread 的结果推到飞书"，规则自动激活。
...
| **L2 · Research OS** | 本仓库 | 骨架、跨项目 wiki、skill、ADR、归档规范 |
...
| 外部平台（飞书 / Notion）归档工作流 | [.claude/rules/feishu-mirror-workflow.md](.claude/rules/feishu-mirror-workflow.md) |
